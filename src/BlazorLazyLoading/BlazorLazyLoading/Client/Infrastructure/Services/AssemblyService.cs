...
namespace BlazorLazyLoading.Client.Services
{
    public class AssemblyService
    {
        ...
        internal List<Assembly> GetLoadedAssemblies(bool reload)
        {
            if (_loadedAssemblies == null || _loadedAssemblies.Count < 1 || reload == true)
            {
                _loadedAssemblies = AppDomain.CurrentDomain
                    .GetAssemblies()
                    .Where(a =>
                        a.FullName != null &&
                        a.FullName.Contains(ModuleIdentifier, StringComparison.OrdinalIgnoreCase))
                    .ToList();
            }
